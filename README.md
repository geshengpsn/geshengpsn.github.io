...
## Univarsal Embodiment Interface 通用具身接口

本项目旨在为具身智能的多模态数据采集与摇操提供一个通用的工具链、可扩展的开源数据与模型。工具链包含了一系列紧密相连的软硬件工具。其中有：数据采集夹爪、机器人末端夹爪、数据可视化、编辑与管理工具、统一机器人控制库。通过使用数据采集夹爪来高效收集灵巧操作数据，收集的数据经过编辑管理步骤成为可供训练的高质量数据，通过开源的数据收集工具链来与合作伙伴一起搭建一个共享共建的大型数据集。使用共建的大数据集，通过模仿学习等方法，可以训练出性能更好的具身智能模型。通过使用本项目开发的机器人夹爪，可以直接将模型迁移到其他机器人上，实现跨具身智能平台的模型迁移。
...
为了保证视觉数据在收集环节与部署环节没有偏移，这就需要设计一款与数据收集夹爪外观相同的机器人夹爪。
